...
def feedforward(layers, inputs, num_units=[2048, 512], is_training=True):
    ...
    #
    #     # Readout layer
    #     params = {"inputs": outputs, "filters": num_units[1], "kernel_size": 1, "activation": tf.nn.relu,
    #               "use_bias": True}
    #     outputs = tf.layers.conv1d(**params)
    #
    #     # Residual connection
...
class TransformerDecoder(keras.layers.Layer):
    ...
    def __init__(self, config, is_train):
        ...
        self.W_embed  = self.add_weight("weights", shape= [1, self.input_dimension, self.input_embed], initializer=self.initializer)
        ...
        self.conv_adj_probab = tf.keras.layers.Conv1D(filters=self.max_length, kernel_size=1, activation=None, use_bias=True)
    ...
    def decode(self, inputs):
        ...
        inputs_with_all_user_embedding = tf.concat([inputs,
                                                    tf.tile(tf.expand_dims(all_user_embedding, 1),
                                                            [1, self.max_length, 1])], -1)

        # with tf.variable_scope("embedding_MCS"):
            # Embed input sequence
            # W_embed = tf.get_variable("weights", [1, self.input_embed, self.input_embed],
            #                           initializer=self.initializer)  # self.input_dimension*2
        ...
        for i in range(self.num_stacks):  # num blocks
            ...
            #
            #     ### Feed Forward
            #     self.enc = feedforward(self.enc, num_units=[self.input_embed, self.input_embed],
            #                            is_training=self.is_training)
            self.enc = multihead_attention(self.the_layers_stack[i], self.enc, num_heads=self.num_heads,
                                           dropout_rate=0.0, is_training=self.is_training)
        ...
        # Readout layer
        # params = {"inputs": self.enc, "filters": self.max_length, "kernel_size": 1, "activation": None,
        #           "use_bias": True}

        self.adj_prob = self.conv_adj_probab(self.enc)
...
# TODO: Commented for migration to TF2
